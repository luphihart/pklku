...
<div class="container-fluid p-0">
        ...
        <div class="card-premium mb-4 p-3 d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between gap-3" style="background-color: var(--bg-card); border-left: 4px solid {{ $isMasaPenilaianOpen ? 'var(--success)' : 'var(--danger)' }} !important;">
            <div class="d-flex align-items-center gap-3">
                <div class="p-2 rounded-circle {{ $isMasaPenilaianOpen ? 'bg-success-light text-success' : 'bg-danger-light text-danger' }} d-flex align-items-center justify-content-center flex-shrink-0" style="width: 44px; height: 44px;">
                    ...
                    @else
                        <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                    @endif
                </div>
                <div>
    ...
    <div class="card-premium p-0 overflow-hidden">
        ...
        <div class="table-responsive d-none d-md-block">
            <table class="table table-hover align-middle mb-0" style="color: var(--text-primary); font-size: 13px; min-width: 820px;">
                    ...
                    @if(auth()->user()->role === 'guru' || auth()->user()->role === 'admin')
                        <div class="pt-2 border-top d-flex align-items-center justify-content-between gap-2" style="border-top-color: var(--border-color) !important;">
                            <button type="button" class="btn btn-sm btn-primary font-heading flex-grow-1 d-flex align-items-center justify-content-center gap-1 py-1.5" data-bs-toggle="modal" data-bs-target="#gradeModal_{{ $p->id }}" style="font-size: 12.5px;">
                                ...
                                <span>{{ $p->penilaianPkl ? 'Edit / Sahkan Nilai' : 'Input Nilai' }}</span>
                            </button>

                            @if($p->penilaianPkl && $p->penilaianPkl->nilai_akhir > 0)
                                <a href="{{ route('laporan.nilai_pdf', $p->id) }}" class="btn btn-sm btn-outline-danger d-flex align-items-center justify-content-center py-1.5 px-2.5" title="Unduh Rapor PDF">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                    </svg>
                                </a>
                                @if(auth()->user()->role === 'admin')
                                    <button type="button" class="btn btn-sm btn-outline-secondary py-1.5 px-2.5" title="Hapus Nilai" onclick="confirmDeleteNilai({{ $p->penilaianPkl->id }}, '{{ addslashes($p->murid?->nama ?? 'Siswa') }}')">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                        </svg>
                                    </button>
        ...
        <!-- Grade Modals (Separated cleanly from table markup) -->
                            ...
                            <form action="{{ route('penilaian.store') }}" method="POST" id="gradeForm_{{ $p->id }}">
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-bordered table-sm align-middle" style="font-size: 12px; color: var(--text-primary); min-width: 500px;">
                                            <thead class="table-light">
                                                <tr>
                                                    <th style="width: 45%;">Tujuan Pembelajaran</th>
                                                    <th style="width: 35%;">Indikator Penilaian</th>
                                                    <th style="width: 20%;" class="text-center">Skor (0-100)</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                @foreach($tps as $tp)
                                                    @php
                                                        $indikators = $tp->indikators;
                                                        $rowCount = $indikators->count();
                                                    @endphp
                                                    @forelse($indikators as $index => $ind)
                                                        ...
                                                        <tr>
                                                            @if($index === 0)
                                                                <td rowspan="{{ $rowCount }}" class="align-top fw-semibold bg-light" style="background-color: var(--bg-canvas) !important;">
                                                                    {{ $tp->nama }}
                                                                    <div class="text-muted font-monospace mt-1" style="font-size: 10px;">ID: {{ $tp->id }}</div>
                                                                </td>
                                                            @endif
                                                            <td>
                                                                <div>{{ $ind->nama }}</div>
